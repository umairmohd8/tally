...
| `components.jsx` | Habit/date/streak utilities + core UI (`HabitRow`, modals) |
| `screens.jsx` | Larger screens & copy (recovery, weekly review, MVD) |
| `app.jsx` | `App` root — wires it together and mounts |

State persists to `localStorage` under `tally-*` keys. On first run, ~70 days of demo habits
are seeded so the UI isn't empty.
